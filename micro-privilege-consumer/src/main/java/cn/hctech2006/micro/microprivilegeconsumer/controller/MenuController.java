package cn.hctech2006.micro.microprivilegeconsumer.controller;
...
import cn.hctech2006.micro.microprivilegeconsumer.service.SysMenuService;
...
public class MenuController  {
    ...
    @ApiOperation(value = "菜单查询列表",notes = "菜单查询\n" +
            "可查询参数：@ApiImplicitParams({\n" +
            "            @ApiImplicitParam(type = \"query\", name = \"id\", value = \"菜单编号\",required = true),\n" +
            "            @ApiImplicitParam(type = \"query\", name = \"name\",value = \"菜单名\"),\n" +
            "            @ApiImplicitParam(type = \"query\", name = \"parentId\",value = \"父菜单ID，一级菜单为0\"),\n" +
    ...
    @CrossOrigin(origins = "*", allowCredentials = "true",allowedHeaders = "*",methods = {RequestMethod.GET, RequestMethod.DELETE, RequestMethod.HEAD, RequestMethod.OPTIONS, RequestMethod.PUT, RequestMethod.POST, RequestMethod.PATCH})
    public HttpResult findAll(Long id , String name, Long parentId, String perms, Byte delFlag){
        return sysMenuService.findAll(id, name, parentId, perms, delFlag);
    }
}
